...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $sms = $_POST['sms'] ?? '';
  $sms_2 = $_POST['sms_2'] ?? '';
  $email = $_POST['email'] ?? ''; // [추가] 이메일 받기 (없으면 빈 문자열)
  $u_level = (int) ($_POST['user_level'] ?? 1);
  ...
  try {
    ...
    if ($result->getInsertedCount() > 0) {
      // 기존 회장/총무 SMS_2 업데이트
      updateLeadersSms2($database, $collection);
      echo "<script>alert('데이터가 저장되었습니다.'); location.href='tel_select.php';</script>";
      exit;
    }

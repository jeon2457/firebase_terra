...
// ❌ [삭제됨] 비로그인 시 강제 리다이렉트 코드 제거 (공개 페이지로 전환)
/*
if (!isset($_SESSION['user_id'])) {
    $_SESSION['redirect_url'] = $_SERVER['REQUEST_URI']; 
    echo "<script>alert('로그인이 필요합니다.'); location.href = 'login_guest.php';</script>";
    exit;
}
*/
...
// 🔹 [추가] 로그인 정보 확인 (비로그인 시 0)
// 외부 접속자는 세션이 없으므로 user_level은 0이 됩니다.
$user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null;
$user_level = isset($_SESSION['user_level']) ? (int) $_SESSION['user_level'] : 0;
$user_name = isset($_SESSION['user_name']) ? $_SESSION['user_name'] : '';

// 접속자 표시 텍스트 생성 (로그인 시에만 표시)
$user_display_text = '';
if ($user_id) {
  // 🔹 [수정] 이름과 직책(remark) DB에서 직접 조회
  try {
    $u_info = $collection->findOne(['id' => $user_id]);
    if ($u_info) {
      $user_name = (string)($u_info['name'] ?? '');
      $user_remark = (string)($u_info['remark'] ?? '');
    }
  } catch (Exception $e) {
  }
  ...
  $user_display_text = ($user_level >= 10) ? "관리자: {$user_name}{$pos}님" : "회원: {$user_name}{$pos}님";
}

// 📌 이코드는 /php/toggle_auth.php 파일과 tel_sms_send.php 파일과 연동되어 있습니다.
...
try {
  ...
  $auth_status = $status_doc ? (int)$status_doc['is_active'] : 1;
} catch (Exception $e) {
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* 🔴 [중요] 고정 색상 컬럼 (NO, 이름, 전화번호, 비고, SMS) - !important로 강제 고정 */
    th.no,
    th.name,
    th.tel,
    th.remark,
    ...
    @media (max-width: 768px) {
      ...
      /* ✅ [핵심 수정] 갭 제거: header(60px) + wrap2(33px) = 93px (미세조정 92px) */
      .table thead th {
        position: sticky;
        top: 92px;
        background: url('./image/background_mable-1.jpg') center center / cover no-repeat !important;
  ...
  <script type="text/javascript">
    window.onload = () => {
      // 🔹 [추가] 3분 후 자동 잠금 (보안 OFF 상태일 때만 작동)
        ...
        <table class="table">
          ...
          <tbody>
            ...
            foreach ($cursor as $row) {
              ...
              // 🔹 관리자(레벨10) OR 레벨5 OR 보안해제 상태면 링크 활성화
              $can_send_sms = ($auth_status == 0 || (isset($user_level) && $user_level >= 5));
